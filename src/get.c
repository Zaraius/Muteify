...
int refresh_access_token(const char *client_id, const char *client_secret, 
                         const char *refresh_token, char *new_access_token, size_t token_size) {
    CURL *curl;
    CURLcode res;
    struct Memory chunk;
    ...
    if (res == CURLE_OK) {
        ...
        if (json) {
            ...
            if (cJSON_IsString(access_token_json) && access_token_json->valuestring) {
                strncpy(new_access_token, access_token_json->valuestring, token_size - 1);
...
char* get_current_playing_type(void) {
    ...
    cJSON *playing_type = cJSON_GetObjectItemCaseSensitive(json, "currently_playing_type");
    if (cJSON_IsString(playing_type) && (playing_type->valuestring != NULL)) {
        ...
        strcpy(playing_type_string, playing_type->valuestring);
